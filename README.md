...
- **問題ファイル(`ocr/mondai/`):精度は高い**(概ね95%以上)。本文・選択肢ともよく読めます。
  ただし固有名詞や旧字、`嘉永→豆永`・`鰊→触/録` 等の誤認識が一部に残ります。
- **解答ファイル(`ocr/kaitou/`):精度は低い**。解答用紙が格子状(グリッド)の表組みのため、
  Tesseract が升目内の数字を取り違えやすく、特に第8回以降の解答欄はレイアウトが崩れます。
  **答えの数字は必ず元PDFで照合してください。**
- 第1回問題(`1_mondai`)は人手(目視)でも内容を確認済みです。

正確な解答番号が必要な場合は、解答PDFを目視で読み取る追加作業を推奨します。

## ディレクトリ構成

```
ocr/
  mondai/   1_mondai.txt 〜 23_mondai.txt   (問題, 各約30,000字)
  kaitou/   1_kaitou.txt 〜 23_kaitou.txt   (解答, 各1ページ)
```
